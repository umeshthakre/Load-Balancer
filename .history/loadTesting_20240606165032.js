...
app.use("/", async (req, res) => {
  let result = "";
  for (let i = 0; i < 10; i++) {
    await axios.get("http://localhost:3000/fast-response").then((res) => {
    ...
    await axios.get("http://localhost:3000/slow-response").then((res) => {
      console.log(res?.data);
      console.log(res.data[res?.data.length - 1]);
      if (res.data[res?.data.length - 1] == 1) {
        a++;
    ...
    await axios.get("http://localhost:3000/very-slow").then((res) => {
      console.log(res?.data);
      console.log(res.data[res?.data.length - 1]);
      if (res.data[res?.data.length - 1] == 1) {
        a++;
